...
impl Color {
    pub fn as_escape(&self, buf: &mut String, last: Self, foreground: bool) {
        // TODO: Optimize? Seems better without it
        if self == &last {
            return;
        }
            ...
            Color::Named(color) => {
                match color {
                    ...
                    &color => {
                        ...
                        match (color < NamedColor::BrightBlack, foreground) {
...
// TODO: Maybe better than the array?
...
#[inline]
fn u8_write_string(buf: &mut String, num: u8) {
    *buf += unsafe { U8_TO_STR.get_unchecked(num as usize) };
}
...
/// Array for fast u8 to str conversion.
const U8_TO_STR: [&str; 256] = [
    "0",
    "1",
    "2",
    "3",
