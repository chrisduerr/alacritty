...
impl<'a> RenderableCellsIter<'a> {
    ...
    fn populate_block_cursor(&mut self) {
        ...
        let mut cursor_cell = original_cell.clone();
...

        {
            ...
            if let Some(width) = c.width() {
                ...
                let num_cols = self.grid.num_cols();
                {
                    ...
                    // TODO: This is the Drop issue!
                    // TODO: Could be solved with a garbage collector
                    let cell = &mut self.grid[&self.cursor.point];
                    if cell.extra.is_none() {
                        unsafe {
                            std::ptr::write(cell, self.cursor.template.clone());
                        }
                    } else {
